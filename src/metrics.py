...
def chrf_metric_evaluatuion():
    ...
    references = [[item["rsl"] for item in dataset["test"]]]
    
    with open("translated_sentences/opus.txt", "rt", encoding="utf-8") as file:
        hypotheses = file.read().split("\n")
    
    metric = CHRF()
    score = metric.corpus_score(hypotheses, references)
    return score.score

def bleu_metric_evaluation() -> dict:
    dataset = load_dataset("Arseney/parallel_corpus_russian_rsl_glosses")
